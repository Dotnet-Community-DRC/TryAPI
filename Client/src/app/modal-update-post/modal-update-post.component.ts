...
export class ModalUpdatePostComponent implements OnInit {
  ...
  submitForm() {
    ...
    this.httpClient
      .put(
        `${API_ENDPOINTS.UPDATE_POST}/${this.postToUpdate.id}`,
        postToUpdateDTO,
        HTTP_OPTIONS
      )
      .subscribe({
        next: (response) => {
          ...
          let updatedPostIndex = this.postService.allPosts.findIndex(
            (post) => post.id == updatedPostFromServer.id
          );

          this.postService.allPosts[updatedPostIndex].title =
            updatedPostFromServer.title;
          this.postService.allPosts[updatedPostIndex].content =
            updatedPostFromServer.content;
          this.postService.allPosts[updatedPostIndex].published =
            updatedPostFromServer.published;
        ...
        error: (error: HttpErrorResponse) => {
          ...
          console.log(
            `Failed to update the post! Response from server: "HTTP statuscode: ${error.status}: ${error.error}"`
          );
        },
      });
  }
